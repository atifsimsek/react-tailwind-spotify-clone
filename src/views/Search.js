...
const Search = () => {
  ...
  useEffect(() => {
    if (favoritesRef.current) {
      const scrollHandle = () => {
        const isEnd =
          Math.round(favoritesRef.current.scrollLeft) +
    ...
    <>
      <section className="mb-4">
        ...
        <div className="relative">
          {prev && (
            <button
              className="w-12 h-12 hover:scale-[1.06] rounded-full -left-6 bg-white text-black flex items-center justify-center absolute  z-10 top-1/2 -translate-y-1/2"
          ...
          {next && (
            <button
              className="w-12 h-12 hover:scale-[1.06] rounded-full -right-6 bg-white text-black flex items-center justify-center absolute  z-10 top-1/2 -translate-y-1/2"
              onClick={slideFavoritesNex}
            >
